...
public class OSSService {
    ...
    public OSSService() {
        ...
        if (endpoint == null || endpoint.isEmpty()) {
            throw new IllegalStateException("ALIYUN_OSS_ENDPOINT environment variable is not set");
        }
        if (accessKeyId == null || accessKeyId.isEmpty()) {
            throw new IllegalStateException("ALIYUN_OSS_ACCESS_KEY_ID environment variable is not set");
        }
        if (accessKeySecret == null || accessKeySecret.isEmpty()) {
            throw new IllegalStateException("ALIYUN_OSS_ACCESS_KEY_SECRET environment variable is not set");
        }
        if (bucketName == null || bucketName.isEmpty()) {
            throw new IllegalStateException("ALIYUN_OSS_BUCKET_NAME environment variable is not set");
        }
        
        // 创建 OSS 客户端
        this.ossClient = new OSSClientBuilder().build(endpoint, accessKeyId, accessKeySecret);
        
        logger.info("OSSService initialized with endpoint: {}, bucket: {}", endpoint, bucketName);
    }
